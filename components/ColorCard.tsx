    ...
    return (
        <div className={`aspect-square w-full h-auto overflow-hidden rounded-lg border-2 relative  ${selectedColor?.hex === colorItem.hex ? 'border-black' : 'hover:border-gray-500 border-lightgray'}`} >
            ...
            >
                ...
                <div className={`h-full w-full flex items-center justify-center z-30 stroke-1 stroke-gray-700 fill-current  ${favoriteColors.includes(colorItem) ? 'fill-gray-700' : 'hover:fill-gray-400 fill-transparent'}`}>
                    <svg className='w-2/3 h-2/3' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            ...
            {selectedColor?.hex === colorItem.hex && (
                <button
                    className="absolute opacity-80 top-1 left-1 w-[30%] h-[30%] bg-transparent z-20 flex items-center justify-center "
